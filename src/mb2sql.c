...
int main(int argc, char *argv[])
{
  ...
  if (!config_read_file(cf,"/etc/config/mb2sql")) {
  ...
  printf("Modbus TCP client service started\n");

  modbus_t *mb;
  uint16_t tab_reg[32];
  mb = modbus_new_tcp("192.168.33.13", 502);
  ...
  int rc = modbus_read_registers(mb, 425, 1, tab_reg);
  if (rc == -1)
  {
    fprintf(stderr, "%s\n", modbus_strerror(errno));
    return -1;
  ...
  printf("Modbus connection close - EXIT\n\n");
